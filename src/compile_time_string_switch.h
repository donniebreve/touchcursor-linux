...
uint32_t uint_crc32_s( const char* str ) {
  return crc32_s(strlen(str)-1,str) ^ 0xFFFFFFFF;
}
...
//convenient macros
#define S_SWITCH( X, ... ) const char* SWITCH_KEY##__VA_ARGS__ = (X == NULL) ? "" : X; switch( int_crc32_s( SWITCH_KEY##__VA_ARGS__ ) )
#define S_FALLTHROUGH( X, ... ) case int_crc32(X): if( strcmp(SWITCH_KEY##__VA_ARGS__,X) ){ goto S_DEFAULT_LABEL##__VA_ARGS__;}
#define S_CASE( X, ... ) break; case int_crc32(X): if( strcmp(SWITCH_KEY##__VA_ARGS__,X) ){ goto S_DEFAULT_LABEL##__VA_ARGS__;}
#define S_DEFAULT( ... ) break; S_DEFAULT_LABEL##__VA_ARGS__: default:
#define S_DEFAULT_FALLTHROUGH( ... ) S_DEFAULT_LABEL##__VA_ARGS__: default:

/*
 * Example Usage of S_SWITCH
#include "rust.h"

int main(int argc, char **argv) {
	//two strings that yield same crc32
	const char* collision_1="DeferredAmbient_6_1_18-1of2_5";
	const char* collision_2="PostEffect_Lighting_18_6-0of1_8_14_13-1of2_19";
	...
	S_SWITCH( argv[1] ) {
		...
		S_CASE("bar")
			println("bar");
		S_CASE("PostEffect_Lighting_18_6-0of1_8_14_13-1of2_19")
			println("Yes!");
//		S_CASE("DeferredAmbient_6_1_18-1of2_5")
//			println("Compilation error - duplicate case!");
		S_DEFAULT()
			println("Just you!");
	}
}
